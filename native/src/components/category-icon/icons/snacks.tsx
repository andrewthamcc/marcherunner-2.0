import React from 'react'
import Svg, { Path } from 'react-native-svg'

export const Snacks: React.FC = () => {
  return (
    <Svg viewBox="0 0 48 48">
      <Path
        fill="#f9e130"
        d="M39,13c0-3.314-2.686-6-6-6c0-1.657-1.343-3-3-3c-1.072,0-2.006,0.567-2.536,1.412 C26.709,3.984,25.227,3,23.5,3c-2.158,0-3.957,1.52-4.394,3.547C18.307,6.197,17.427,6,16.5,6C12.91,6,10,8.91,10,12.5 c0,2.3,1.198,4.317,3.001,5.472C13.001,17.982,13,17.991,13,18c0,6.075,4.925,11,11,11c5.851,0,10.622-4.572,10.966-10.336 C37.313,17.849,39,15.624,39,13z"
      ></Path>
      <Path
        fill="#f44336"
        d="M35.7,43L39,18.31h-0.001c0.039-1.555-1.116-2.841-2.567-2.843 c-1.453-0.002-2.664,1.258-2.706,2.815c0,0.01,0.002,0.019,0.002,0.028h-6.486C27.2,16.732,25.768,15.462,24,15.462 s-3.2,1.27-3.242,2.848h-6.49c0-0.01,0.002-0.018,0.002-0.028c-0.042-1.557-1.252-2.818-2.703-2.815 c-1.45,0.002-2.604,1.288-2.565,2.843H9L12.3,43H35.7z"
      ></Path>
      <Path
        fill="#e1f5fe"
        d="M33.98 18.319l-2.048 22.605h-5.446l.685-22.718c0-.016-.006-.03-.006-.046 0-1.49 1.53-2.698 3.418-2.698 1.859 0 3.365 1.174 3.41 2.633H34l-.003.038c0 .009.003.018.003.027C34 18.215 33.984 18.266 33.98 18.319zM14.02 18.319l2.048 22.605h5.446L20.83 18.207c0-.016.006-.03.006-.046 0-1.49-1.53-2.698-3.418-2.698-1.859 0-3.365 1.174-3.41 2.633H14l.003.038c0 .009-.003.018-.003.027C14 18.215 14.016 18.266 14.02 18.319z"
      ></Path>
      <Path
        fill="#f44336"
        stroke="#f44336"
        stroke-miterlimit="10"
        stroke-width="2"
        d="M14.308 41.962L32.615 41.962"
      ></Path>
      <Path
        fill="none"
        stroke="#ffb829"
        stroke-linecap="round"
        stroke-linejoin="round"
        stroke-miterlimit="10"
        stroke-width="2"
        d="M32.733,12.639c-0.536-1.306-1.82-2.226-3.319-2.226c-0.509,0-0.991,0.109-1.429,0.3c-0.539-1.562-2.006-2.691-3.751-2.691 c-2.201,0-3.985,1.784-3.985,3.985c0,0.017,0.005,0.033,0.005,0.05c-0.507-0.521-1.213-0.847-1.998-0.847 c-1.038,0-1.943,0.567-2.424,1.408"
      ></Path>
    </Svg>
  )
}
